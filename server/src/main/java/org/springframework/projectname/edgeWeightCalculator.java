...
public class edgeWeightCalculator {
    ...
    public void calculate() {
        ...
        while ((currentIdx) != vtxs.size()) {
            while ((cursor) != vtxs.size()) {
                ...
                System.out.println("Address: " + e.getVertex1().getAddress().toString() + " "
                        + e.getVertex2().getAddress().toString() + "\n" + "Distance: " + distance + "\n");

                if (distance < 0) {
                    // an error occured
                    distance = Double.MAX_VALUE;
                    System.out.print("An error has occured calculating the distance between: "
                            + e.getVertex1().getAddress() + " and " + e.getVertex2().getAddress());
    ...
    private double getWeight(tudeAddress source, tudeAddress destination) {
        // returns the distance from point a to b if valid addresses are passed
        // otherwise returns a non positave number and set the error status of this
        // object
